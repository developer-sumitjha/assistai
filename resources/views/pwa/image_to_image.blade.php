<x-pwa-layout>
    ...
    <div class="flex flex-col min-h-screen bg-[#F2F2F2]" x-data="imageToImageGenerator()">
        ...
        <x-pwa.top-nav>
            <x-slot:left>
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 class="text-[13px] font-bold text-slate-800 tracking-tight">Image-to-Image</h1>
                        <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest leading-none">AI
                            Creation</p>
                    </div>
                </div>
            </x-slot:left>
        ...
        <main class="flex-1 p-6 space-y-6 pb-32 overflow-y-auto" style="padding-top: 120px !important;">
            ...
            <div class="bg-white rounded-[32px] p-6 shadow-xl shadow-slate-200/50 border border-slate-100">
                <div class="space-y-4">
                    <!-- Image Upload -->
                    <div>
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Upload Source Image</label>
                        <div class="relative group">
                            <input type="file" id="image-upload" @change="handleFileUpload" class="hidden" accept="image/*">
                            <label for="image-upload" class="flex flex-col items-center justify-center w-full aspect-video rounded-2xl border-2 border-dashed border-slate-100 bg-slate-50 cursor-pointer hover:bg-slate-100 transition-colors overflow-hidden relative">
                                <template x-if="!imagePreview">
                                    <div class="flex flex-col items-center py-4">
                                        <i data-lucide="upload-cloud" class="w-8 h-8 text-slate-300 mb-2"></i>
                                        <p class="text-[11px] font-bold text-slate-400">Click to upload image</p>
                                    </div>
                                </template>
                                <template x-if="imagePreview">
                    ...
                    <div>
                        ...
                        <textarea x-model="prompt"
                            ...
                            placeholder="Make this image in a cyberpunk style, or add a sunset background..."></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <select x-model="selectedModel"
                                class="w-full bg-slate-50 border-none rounded-xl py-3 px-4 text-xs font-bold text-slate-600 focus:ring-2 focus:ring-blue-500">
                    ...
                    <div x-data="{ expanded: false }">
                        ...
                        <div x-show="expanded" x-collapse x-cloak class="pt-4 grid grid-cols-2 gap-4">
                            <div>
                                <label
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Aspect
                                    Ratio</label>
                            ...
                            <div>
                                <label
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Quality</label>
                                <select x-model="imageSize"
                                    class="w-full bg-slate-50 border-none rounded-xl py-2 px-3 text-[10px] font-bold text-slate-600 focus:ring-2 focus:ring-blue-500">
                    ...
                    <!-- Generate Button -->
                    <button @click="generateImage()" :disabled="loading || !prompt || !imageFile"
            ...
            <div x-show="generatedImage" x-transition x-cloak class="space-y-4">
                <div class="bg-white rounded-[32px] overflow-hidden shadow-xl border border-slate-100 p-2">
                    ...
                    <div class="p-4 flex items-center justify-between">
                        <p class="text-[10px] font-bold text-slate-400 italic truncate max-w-[200px]"
                            x-text="'\"' + prompt + ' \"'"></p>
            ...
            <div class="space-y-4 pt-4">
                <div class="flex items-center justify-between px-2">
                    <h3 class="text-sm font-black text-slate-800 uppercase tracking-tight">Recent Transformations</h3>
                    <span
                ...
                <div class="grid grid-cols-2 gap-4">
                    @foreach($generations as $gen)
                        <div class="aspect-square bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm relative group cursor-pointer"
                            @click="viewHistory('{{ $gen->imageUrl }}', '{{ addslashes($gen->prompt) }}')">
                            <img src="{{ $gen->imageUrl }}"
                                class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                alt="History">
                            <div
                                class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3">
        ...
        <!-- Toast Notification -->
        <div x-show="toast.show" x-transition x-cloak class="fixed top-24 left-1/2 -translate-x-1/2 z-[10000] px-6 py-3 rounded-2xl shadow-2xl flex items-center space-x-3 min-w-[280px]" :class="toast.type === 'success' ? 'bg-black text-white' : 'bg-red-500 text-white'">
    ...
    <script>
        ...
        function imageToImageGenerator() {
            return {
                ...
                async generateImage() {
                    if (!this.prompt || !this.imageFile) return;
                    ...
                    formData.append('image_size', this.imageSize);

                    try {
                        const response = await fetch('{{ route("user.image_to_image.generate") }}', {
                            method: 'POST',
                            headers: {
                        ...
                        if (data.success) {
                            ...
                            this.showToast('Transformation complete!');
